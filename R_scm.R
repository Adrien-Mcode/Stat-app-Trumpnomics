...
#Apres avoir importer la base, on observe sa forme puis on met la variable "TIME" au format numerique.
...
#Creation de la liste des predicteurs
...
#qui va preparer les vecteurs et matrices utilisees pour trouver la ponderation. 
#x1 est le vecteur pour l'unite traite (les Etats-Unis) des 3 series temporelles et des 5 covariables, 
#X0, la matrice equivalente pour les unites de controle (le donor pool), 
#Z1 le vecteur de la variable d'interet (ici le taux de chomage) pour l'unite traite 
#et Z0 la matrice de la variable d'interet pour chacun des pays du donor pool.



##########   Creation des vecteurs du probleme d'optimisation    ###########
...
dataprep.out$Z1  #Vecteur du taux de chomage US sur la periode d'entrainement
dataprep.out$Z0  #Matrice des taux de chomage des unites de controle sur la periode d'entrainement


##########   Resolution du probleme d'optimisation et resultats    ###########
#Resolution du probleme de minisation a l'aide de la fonction synth() du package "Synth".
#Elle nous donne en sortie les valeurs predites par le controle synthetique pour chacune des variables de la minimisation, 
#la ponderation W du controle synthetique et la ponderation V des variables.
...
#Ecart entre le taux de chomage predit par le controle synthetique et celui observe des US

head(synth.tables$tab.pred, 5) #Tableau des predictions comparees aux valeurs observees
head(synth.tables$tab.v, 5)    #Tableau des ponderations des variables
synth.tables$tab.w            #Tableau des ponderations des pays


##########   Graphiques du modele baseline    ###########
...
#Pour le taux de chomage, le PIB, l'emploi et les actifs, on trace les courbes obtenus pour les Etats-Unis et le controle synthetique,
#l'intervalle de confiance etant obtenu par + ou - 2 fois l'ecart-type de la difference entre les vraies valeurs 
#et celles predites sur la periode pre-traitement.
...
legend(1995, 1, legend=c("USA", "controle synthetique", "election de Trump"), col=c("blue", "red", "green"), lty=1)

#Plot du chomage
chom = graph_bl$sc_cho - 2*sd(graph_bl$USA_cho - graph_bl$sc_cho)
chop = graph_bl$sc_cho + 2*sd(graph_bl$USA_cho - graph_bl$sc_cho)

plot(data$TIME[1:101], graph_bl$USA_cho, ylab = 'Taux de chomage', xlab = 'Time', ylim = c(0,10), col="blue", type = "l", lty = 1, lwd=4)
...
abline(v = 2016.75, col="green", lwd=3, lty=2)
legend(1995, 10, legend=c("USA", "controle synthetique", "election de Trump"), col=c("blue", "red", "green"), lty=1)
...
abline(v = 2016.75, col="green", lwd=3, lty=2)
legend(1995, 0.8, legend=c("USA", "controle synthetique", "election de Trump"), col=c("blue", "red", "green"), lty=1)
...
legend(1995, 10, legend=c("USA", "controle synthetique", "election de Trump", "leave-one-out"),
...
plot_placebos(
  ...
  mspe.limit = 20,
  xlab = 'Time',
  ylab = 'Difference de taux de chomage entre predicteurs et vraies valeurs pour chacun des pays',
...
# on reprend le code du modele baseline en inversant chomage et PIB
...
plot(data$TIME[2:101], data$Chomage[1718:1817], ylab = 'Taux de chomage', xlab = 'Time', ylim = c(0,10), col="blue", type = "l", lty = 1, lwd=4)
...
# Tests de robustesse pour le modele alternatif
...
plot_placebos(
  ...
  mspe.limit = 20,
  xlab = 'Time',
  ylab = 'Difference de PIB entre predicteurs et vraies valeurs pour chacun des pays',
  title = "Placebo PIB",
  alpha.placebos = 1,
)
